...
class AccesoDatos
{
    ...
    public function compruebausuario($login, $contraseña)
    {
        ...
        $stmt_usuarios->bindParam(1, $login, PDO::PARAM_STR);
        $stmt_usuarios->bindParam(2, $contraseña, PDO::PARAM_STR);
    ...
    public function noexisteEmail(string $email): bool
    {
        ...
        $existe = true;
        // Crea la sentencia preparada
        $stmt_usuarios  = $this->dbh->prepare("select * from Clientes where email=:email");
        // Si falla termina el programa
        $stmt_usuarios->setFetchMode(PDO::FETCH_CLASS, 'Cliente');
        // Ejecuto la sentencia
        // $stmt_usuarios->bindParam(1, $email);
        $stmt_usuarios->bindValue(':email', $email);
        // Obtengo los resultados
        if ($stmt_usuarios->execute()) {
